...
def buildModel(categories, data):
    # create model
    model = Sequential()
    model.add(Dense(96, input_dim=(data.shape[1]), activation='relu'))
...
myData = X_all_features
model = buildModel(10, myData)
model.fit(myData, y, epochs=100)

model.predict(myData[25,:], y, verbose=1)

# sklearn estimator build
# estimator = KerasClassifier(build_fn=class10_model, epochs=50, batch_size=256, verbose=1)
# kfold = KFold(n_splits=3, shuffle=True, random_state=seed)
# results = cross_val_score(estimator, X_selected, dummy_y, cv=kfold)
